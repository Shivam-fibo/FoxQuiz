  ...
  const QuizDetails = () => {
    ...
    const submitQuiz = async () => {
      ...
      try {
        ...
        const feedbackData = quiz.questions.map((question, index) => ({
          questionText: question.questionText,
          correctAnswer: question.correctAnswer,
          userAnswer: answers[index] 
        }))
        setTimeout(() => {
          navigate("/feedback", { state: { score, totalQuestion, feedbackData } });
        }, 1000);
      } catch (error) {
  ...
  {currentQuestionIndex < quiz?.questions.length - 1 ? (
    <button
      ...
      className="bg-red-500 text-white px-4 py-2 rounded-md hover:bg-purple-600 disabled:bg-gray-300 
                 sm:px-3 sm:py-1 sm:text-sm" // Smaller size for small screens
    ...
    <button
      ...
      className="bg-gradient-to-r from-blue-500 to-blue-700 text-white px-4 py-2 rounded-md 
                 sm:px-3 sm:py-1 sm:text-sm disabled:bg-gray-300" // Smaller size for small screens
      disabled={!selectedAnswers[id]?.[currentQuestionIndex]}
    >
      Submit
    </button>
